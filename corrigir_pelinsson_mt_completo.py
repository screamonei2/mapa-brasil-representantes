...
# -*- coding: utf-8 -*-
"""
Script para corrigir e adicionar todas as cidades faltantes do Mato Grosso (MT)
para o representante Pelinsson (ID 33).
...
def carregar_cidades_mt():
    """
    Carrega todas as cidades do Mato Grosso do arquivo municipios.json
    """
    print("Carregando todas as cidades do Mato Grosso...")
    ...
    for feature in data['features']:
        if feature['properties']['UF'] == 'MT':
            ...
            mt_cities.append(city_name_normalized)
    
    # Remove duplicatas e ordena
    mt_cities = sorted(list(set(mt_cities)))
    
    print(f"Total de cidades encontradas no MT: {len(mt_cities)}")
    return mt_cities
...
def fazer_backup(arquivo):
    ...
    backup_name = f"{arquivo.replace('.json', '')}_backup_pelinsson_mt_{timestamp}.json"
    shutil.copy2(arquivo, backup_name)
    print(f"Backup criado: {backup_name}")
    return backup_name

def corrigir_pelinsson_mt():
    """
    Função principal para corrigir as cidades do representante Pelinsson no MT
    """
    arquivo_representantes = 'old/representantes_por_estado.json'
    
    print("=== Correção Pelinsson MT - Todas as Cidades ===")
    print()
    
    # 1. Fazer backup
    backup_file = fazer_backup(arquivo_representantes)
    
    # 2. Carregar todas as cidades do MT
    todas_cidades_mt = carregar_cidades_mt()
    ...
    # 4. Encontrar o representante Pelinsson
    pelinsson_key = None
    ...
    if not pelinsson_key:
        print("ERRO: Representante Pelinsson (código 33.0) não encontrado!")
        return False
    
    print(f"Representante encontrado: {data['representantes'][pelinsson_key]['nome']}")
    ...
    print(f"Cidades atuais do Pelinsson: {len(cidades_atuais)}")
    ...
    if len(cidades_faltantes) == 0:
        print("Pelinsson já atende todas as cidades do MT!")
        return True
    ...
    print()
    print("Cidades atuais do Pelinsson:")
    ...
    todas_cidades_final = sorted(list(set(cidades_atuais_normalized + cidades_faltantes)))
    
    # 10. Atualizar dados do representante
    data['representantes'][pelinsson_key]['estados']['MT']['cidades'] = todas_cidades_final
    data['representantes'][pelinsson_key]['estados']['MT']['total_cidades'] = len(todas_cidades_final)
    data['representantes'][pelinsson_key]['total_cidades'] = len(todas_cidades_final)
