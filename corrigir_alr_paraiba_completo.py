...
# -*- coding: utf-8 -*-
"""
Script para corrigir e adicionar TODAS as cidades da Paraíba (PB)
para o representante ALR Consultoria (ID 19.0).
...
def carregar_cidades_pb():
    """
    Carrega todas as cidades da Paraíba do arquivo municipios.json
    """
    print("Carregando todas as cidades da Paraíba...")
    ...
    for feature in data['features']:
        if feature['properties']['UF'] == 'PB':
            ...
            pb_cities.append(city_name_normalized)
    
    # Remove duplicatas e ordena
    pb_cities = sorted(list(set(pb_cities)))
    
    print(f"Total de cidades encontradas na PB: {len(pb_cities)}")
    return pb_cities
...
def fazer_backup(arquivo):
    ...
    backup_name = f"{arquivo.replace('.json', '')}_backup_alr_pb_{timestamp}.json"
...
def corrigir_alr_pb():
    """
    Função principal para adicionar todas as cidades faltantes da PB ao representante ALR
    """
    arquivo_representantes = 'old/representantes_por_estado.json'
    
    print("=== Correção ALR PB - Todas as Cidades ===")
    print()
    
    # 1. Fazer backup
    backup_file = fazer_backup(arquivo_representantes)
    
    # 2. Carregar todas as cidades da PB
    todas_cidades_pb = carregar_cidades_pb()
    ...
    # 4. Encontrar o representante ALR
    alr_key = None
    ...
    if not alr_key:
        print("ERRO: Representante ALR (código 19.0) não encontrado!")
        return False
    
    print(f"Representante encontrado: {data['representantes'][alr_key]['nome']}")
    ...
    print(f"Cidades atuais do ALR: {len(cidades_atuais)}")
    ...
    if len(cidades_faltantes) == 0:
        print("ALR já atende todas as cidades da PB!")
        return True
    ...
    todas_cidades_pb_final = sorted(list(set(cidades_atuais_normalized + cidades_faltantes)))
    
    # 10. Atualizar dados do representante
    data['representantes'][alr_key]['estados']['PB']['cidades'] = todas_cidades_pb_final
    data['representantes'][alr_key]['estados']['PB']['total_cidades'] = len(todas_cidades_pb_final)
    data['representantes'][alr_key]['total_cidades'] = len(todas_cidades_pb_final)
